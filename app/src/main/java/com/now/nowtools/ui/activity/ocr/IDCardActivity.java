 ...
 */
package com.now.nowtools.ui.activity.ocr;
...
import com.now.nowtools.utils.FileUtil;
                ...
                if (result != null) {
                    if (idCardSide.equals(IDCardParams.ID_CARD_SIDE_FRONT)) {
                        String res = String.format("姓名：%s\n性别：%s   民族：%s \n出生：%s\n住址：%s\n公民身份号码：%s",
                                result.getName().getWords(),
                                result.getGender().getWords(),
                                result.getEthnic().getWords(),
                                result.getBirthday().getWords(),
                                result.getAddress().getWords(),
                                result.getIdNumber().getWords());
                        infoTextView.setText(res);
                    } else {
                        String res = String.format("签发机关：%s\n有效期限：%s  ",
                                result.getIssueAuthority().getWords(),
                                result.getSignDate().getWords() + "-" + result.getExpiryDate().getWords());
                        infoTextView.setText(res);
                    }
